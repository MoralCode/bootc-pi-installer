...
sudo podman run --rm -it --privileged --pull=newer --security-opt label=type:unconfined_t -v /config.toml:/config.toml:ro -v /output:/output ghcr.io/moralcode/bootc-image-builder:tutorial --type raw --target-arch aarch64 --rootfs ext4 --partition-table-variant rpi --local "$1"
