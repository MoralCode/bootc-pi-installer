...
sudo podman run --rm -it --privileged --pull=newer --security-opt label=type:unconfined_t -v /config.toml:/config.toml:ro -v /output:/output -v /var/lib/containers/storage:/var/lib/containers/storage "${BIB_CONTAINER}" --type raw --target-arch aarch64 --rootfs ext4 --partition-table-variant rpi --local "$1"
